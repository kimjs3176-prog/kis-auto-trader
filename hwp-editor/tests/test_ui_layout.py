...
def test_위에_붙이면_넓적하게_눕는다():
    자리 = layout.창자리계산("상", 화면너비, 화면높이, 두께=300, 여백=48)
    assert (자리.너비, 자리.높이, 자리.왼쪽, 자리.위) == (화면너비, 300, 0, 0)


def test_아래에_붙이면_작업표시줄_자리를_비워_둔다():
    자리 = layout.창자리계산("하", 화면너비, 화면높이, 두께=300, 여백=48)
    assert 자리.위 + 자리.높이 == 화면높이 - 48


def test_네_자리_모두_계산된다():
...
def test_모르는_자리는_거절한다():
    try:
        layout.창자리계산("가운데", 화면너비, 화면높이, 두께=320)
    except ValueError:
        return
    raise AssertionError("모르는 자리를 받아들였습니다.")
...
def test_두께는_너무_얇거나_두꺼워지지_않는다():
    ...
    assert layout.두께다듬기("하", 5000, 화면너비, 화면높이) <= int(화면높이 * layout.최대두께비)
