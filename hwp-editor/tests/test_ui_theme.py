"""배색 시험 — 시안(ProDoc Assistant)의 색 규칙이 지켜지는지.

색은 눈으로 봐야 아는 것이지만, 적어도 (1) 색 코드가 성한 값인지, (2) 색 섞기가
맞게 되는지, (3) 글자와 바탕이 너무 비슷해 안 보이지는 않는지, (4) 채움 칸이
카드와 구별되는지는 기계가 볼 수 있다.
"""
...
# ---------------------------------------------- 시안: 테두리 대신 색으로 채운 칸
def test_입력칸은_카드와_구별되고_글자는_읽힌다():
    """시안은 입력칸을 테두리로 가두지 않고 옅은 파랑으로 채운다."""
    assert _밝기(theme.입력칸) < _밝기(theme.카드), "흰 카드 위에서 칸이 안 보입니다"
    assert _밝기(theme.입력칸) - _밝기(theme.진한글) > 0.6, "채운 칸 안의 글자가 묻힙니다"


def test_결과칸은_입력칸보다_옅다():
    """읽기만 하는 칸이라 입력칸만큼 진할 이유가 없다."""
    assert _밝기(theme.결과칸) > _밝기(theme.입력칸)
    assert _밝기(theme.결과칸) < _밝기(theme.카드)


def test_주_파랑은_흰_글자를_얹을_만큼_진하다():
    """`실행` 단추와 고른 타일 배지에 흰 글자를 쓴다."""
    assert _밝기(theme.카드) - _밝기(theme.파랑) > 0.6


def test_못_누르는_파랑은_주_파랑보다_옅다():
    assert _밝기(theme.꺼짐) > _밝기(theme.파랑)


def test_분류_강조색도_흰_글자를_얹을_수_있다():
    """고르면 배지가 강조색으로 꽉 차고 그 위에 글머리를 흰색으로 얹는다."""
    from hwpkit import commands

    for 이름 in commands.분류순서:
        _글머리, 강조 = theme.분류모습(이름)
        assert _밝기(theme.카드) - _밝기(강조) > 0.35, (이름, 강조)
