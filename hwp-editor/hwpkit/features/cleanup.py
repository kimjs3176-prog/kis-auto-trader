...
def 서식정리(
    ...
    `기준` 에 회사 양식 이름(`doc_format.기준이름들()`)을 주면 서식을 지운 뒤
    그 양식의 편집용지·본문·글머리 수준 서식을 **입힌다**. 지우기만 하면
    한/글 기본값(함초롬바탕 10pt)이 되어 양식과 더 멀어지기 때문이다.
    """
    한일: list[str] = []
    if 문단초기화:
        문단서식초기화(문서객체)
    ...
    if 기준:
        ...
        요약 = doc_format.기준적용(문서객체, 기준, 제목포함=제목포함)
        한일.extend(요약.한일)
    if 표초기화:
        수 = 표서식초기화(문서객체)
        한일.append(f"표 {수}개")
