...
# ---------------------------------------------------------------- 실행판
#: 실행판 입력칸에 늘 남겨 두는 높이(px) — 한 줄 입력 하나가 들어갈 만큼
입력칸최소 = 60
#: 입력칸이 실행판을 차지할 수 있는 최대 비율. 나머지는 결과칸 몫이다.
입력칸최대비 = 0.55
#: 아래쪽 세부설정 판이 가질 최소 높이(px)
세부판최소 = 190
#: 세부설정 판이 가운데 칸에서 차지할 수 있는 최대 비율. 나머지는 타일 격자 몫이다.
세부판최대비 = 0.70


def 입력칸높이(요구높이: int, 창높이: int) -> int:
    """실행판 입력칸에 줄 높이(px).

    입력이 하나뿐인 기능이 대부분이라 입력칸을 크게 잡으면 결과가 눌린다.
    반대로 입력이 여섯 개인 기능(문서 정리)에서는 두 줄만 보여 스크롤해야 한다.
    그래서 **필요한 만큼** 주되, 결과칸 몫을 절반 이상 남긴다.
    """
    위한계 = max(입력칸최소, int(max(창높이, 0) * 입력칸최대비))
    return max(입력칸최소, min(max(요구높이, 0), 위한계))


def 세부판높이(요구높이: int, 가운데높이: int) -> int:
    """아래쪽 세부설정 판에 줄 높이(px).

    판이 필요로 하는 만큼 주되, 타일 격자가 아주 눌리지 않게 위를 막는다.
    격자를 그대로 보여 주는 값이므로 판이 화면을 다 먹으면 뜻이 없어진다.
    """
    if 가운데높이 <= 0:  # 아직 창 크기를 모르는 때
        return max(요구높이, 세부판최소)
    위한계 = max(세부판최소, int(가운데높이 * 세부판최대비))
    return max(세부판최소, min(max(요구높이, 0), 위한계))


def 격자칸높이(요구높이: int, 가운데높이: int, 최소세부판: int = 세부판최소) -> int:
    """세부설정 판을 띄운 동안 타일 격자가 쓸 높이(px).

    타일을 눌렀을 때 격자를 치우지 않고 **아래쪽 빈자리에** 세부설정을 띄운다.
    타일이 몇 개뿐이면 격자를 필요한 만큼만 쓰게 해서, 남는 자리가 통째로
    세부설정 몫이 되게 한다(가운데가 텅 비지 않는다). 타일이 많으면 격자는
    남은 자리까지만 쓰고 그 안에서 구른다.
    """
    if 가운데높이 <= 0:  # 아직 창 크기를 모르는 때
        return max(요구높이, 0)
    남음 = max(0, 가운데높이 - 최소세부판)
    return max(0, min(max(요구높이, 0), 남음))
